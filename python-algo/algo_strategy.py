...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def starter_strategy(self, game_state):
        ...
        if game_state.turn_number < 5:
            ...
            # Only spawn Scouts every three turns
            # Sending more at once is better since attacks can only hit a single scout at a time
            if game_state.turn_number % 3 == 1:
                # To simplify we will just check sending them from back left and right
                best_location = self.get_low_damage_pos(game_state)
    ...
    def build_defences(self, game_state):
        ...
        if game_state.turn_number == 1:
            ...
            wall_locations = final_wall

        game_state.attempt_spawn(WALL, wall_locations)

        weak_walls = [[0, 13], [1, 12], [2, 11], [3, 10], [4, 9]]
        game_state.attempt_upgrade(weak_walls)
